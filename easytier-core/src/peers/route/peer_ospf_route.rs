...
use crate::{
    ...
    proto::{
        common::{NatType, RuntimeTimestamp as Timestamp, TimestampExt},
        core_peer::peer::{
...
struct SyncedRouteInfo {
    ...
    // Keep the original protobuf bytes so unknown fields survive multi-hop propagation.
    raw_peer_infos: DashMap<PeerId, RawRoutePeerInfo>,
...
impl SyncedRouteInfo {
    ...
    fn credential_proof_is_valid(
        &self,
        raw_route_info: Option<&RawRoutePeerInfo>,
        proof_idx: usize,
        ...
        network_secret
            .map(|secret| {
                let Some(raw_route_info) = raw_route_info else {
                    return proof.verify_credential_hmac(secret);
                };
                match raw_credential_bytes(raw_route_info, proof_idx) {
                    Ok(Some(raw_credential_bytes)) => {
...
impl PeerRouteServiceImpl {
    ...
    fn build_sync_route_raw_req(
        ...
        raw_peer_infos: &DashMap<PeerId, RawRoutePeerInfo>,
    ) -> Result<Vec<u8>, route_peer_wire::WireError> {
        let infos = req
            .peer_infos
            .as_ref()
            .map(|peer_infos| {
                peer_infos
                    ...
                    .map(|peer_info| {
                        raw_peer_infos
                            ...
                            .map(|raw| raw.clone())
                            .unwrap_or_else(|| raw_route_peer_info(peer_info))
                    })
                    .collect::<Vec<_>>()
            })
            .unwrap_or_default();
        route_peer_wire::encode_sync_route_request(req, &infos)
    }
    ...
    async fn sync_route_with_peer(
        ...
        let raw_request = match Self::build_sync_route_raw_req(
            &sync_route_info_req,
            &self.synced_route_info.raw_peer_infos,
        ) {
            Ok(raw_request) => raw_request,
            Err(error) => {
                tracing::error!(?error, "failed to encode raw OSPF route request");
                return false;
            }
        };
        ctrl.set_raw_input(raw_request.into());
        ...
        let raw_peer_infos = if let Some(peer_infos_ref) = &peer_infos {
            let raw_input = ctrl.get_raw_input().ok_or_else(|| {
                rpc_types::error::Error::MalformatRpcPacket(
                    "OSPF route request is missing raw protobuf input".to_owned(),
                )
            })?;
            let r = extract_route_peer_infos(&raw_input).map_err(anyhow::Error::new)?;
            let raw_matches_decoded = r.len() == peer_infos_ref.len()
                && r.iter().zip(peer_infos_ref).all(|(raw, decoded)| {
                    RoutePeerInfo::decode(raw.clone())
                        .map(|raw_decoded| raw_decoded.peer_id == decoded.peer_id)
                        .unwrap_or(false)
                });
            if !raw_matches_decoded {
                return Err(rpc_types::error::Error::MalformatRpcPacket(
                    "raw and decoded OSPF RoutePeerInfo values do not match".to_owned(),
                ));
            }
            Some(r)
        } else {
...
impl RouteSessionManager {
    ...
    async fn session_task(
        ...
        loop {
            loop {
                ...
                crate::foundation::time::sleep(Duration::from_millis(retry_delay_ms)).await;
    ...
    async fn maintain_sessions(&self, service_impl: Arc<PeerRouteServiceImpl>) -> bool {
        ...
        loop {
            ...
            select! {
                _ = crate::foundation::time::sleep(Duration::from_millis(next_sleep_ms)) => {}
                _ = recv.recv() => {}
            }
    ...
    fn extract_credential_peer_info(
        ...
        let raw_info = patch_credential_route_peer_info(&raw_info, &info.proxy_cidrs).ok()?;
        Some((info, raw_info))
    }
    ...
    async fn do_sync_route_info(
        ...
        raw_peer_infos: Option<Vec<RawRoutePeerInfo>>,
...
impl PeerRoute {
    ...
    async fn clear_expired_peer(service_impl: Arc<PeerRouteServiceImpl>) {
        loop {
            crate::foundation::time::sleep(Duration::from_secs(60)).await;
